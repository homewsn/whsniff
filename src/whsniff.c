...
static int packet_handler(unsigned char *buf, int cnt) {
  ...
  case 0:
    ...
    if (buf[sizeof(usb_data_header_type) + usb_data_header->wpan_len - 1] & 0x80) {
      ...
      if(dbglog) fprintf(dbglog, "crc ok:%x %02x  ",htole16(fcs),buf[sizeof(usb_data_header_type) + usb_data_header->wpan_len - 1]);
    } else {
      if(dbglog) fprintf(dbglog, "crc bad ");
    }
    le_fcs = htole16(fcs);
    ...
    if(dbglog) {
      for(int i=0;i<usb_data_header->wpan_len;i++) {
	fprintf(dbglog, "%02x ",buf[sizeof(usb_data_header_type)+i]);
      }
      fprintf(dbglog,"\n");
    }      
    tiudp *ti = malloc(sizeof(tiudp)+usb_data_header->wpan_len);
    ti->timeStamp = timestamp_us * 32;
    ti->packetLen = usb_data_header->wpan_len;
    memcpy(ti->data, &buf[sizeof(usb_data_header_type)], usb_data_header->wpan_len);
    //memcpy(&ti->data[usb_data_header->wpan_len - 2], &le_fcs, 2); // set CRC
    sendUDP(ti);
    free(ti);
...
void print_usage()
{
  printf("Usage: whsniff -c channel -h host\n");
}
